...
CIUDADES = [
    ...
    "Ibagué", "Neiva", "Villavicencio", "Tunja", "Popayán", "Sincelejo",
    "Montería", "Armenia", "Pasto", "Valledupar", "Riohacha", "Soledad",
    "Palmira", "Itagüí", "Rionegro", "Envigado", "Soacha"
]

# Ruta opcional a un archivo PNG del logo oficial (si lo tienes, pon la ruta aquí)
LOGO_PATH = None  # ej: "/path/to/ministerio_logo.png"
...
def load_font(paths_sizes=[("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 120),
                           ("/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf", 120),
                           ("C:\\Windows\\Fonts\\FE-FONT.TTF", 120)]):
...
def generar_placa_img(codigo, ciudad):
    ...
    slots = [
        (int(WIDTH * 0.12), 18),               # superior izquierda
        (int(WIDTH * 0.88), 18),               # superior derecha
        (int(WIDTH * 0.12), HEIGHT - 18),      # inferior izquierda
        (int(WIDTH * 0.88), HEIGHT - 18)       # inferior derecha
    ]
    for (cx, cy) in slots:
        draw_slot(cx, cy, w_slot=40, h_slot=12, radius=6)

    # Preparar fuentes (buscamos una existente)
    font_path = load_font()
    # Reservar espacio para logo central
    logo_w = 36
    logo_gap = logo_w + 12  # espacio total reservado entre los grupos
    ...
    for size in [140, 130, 120, 110, 100, 90, 80, 72, 64]:
        try:
    ...
    y_text = (HEIGHT * 0.5) - max(left_h, right_h)/2 - 6
    ...
    for size_city in [36, 32, 30, 28, 26, 24]:
        try:
