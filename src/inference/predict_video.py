...
def draw_labels(image, bbox, track_id, score, ocr_text="", color=(0, 255, 0)):
    ...
    if ocr_text:
        # Formato explícito solicitado: "Placa: XXXXX"
...
def process_video(model, video_path, out_video_path=None, img_size=IMG_SIZE[0], display=False,
    ...
    while True:
        ...
        for (t_idx, d_idx) in matches:
            ...
            tr['bbox'] = [
                int(alpha * tr['bbox'][0] + (1 - alpha) * det['bbox'][0]),
                int(alpha * tr['bbox'][1] + (1 - alpha) * det['bbox'][1]),
                int(alpha * tr['bbox'][2] + (1 - alpha) * det['bbox'][2]),
                int(alpha * tr['bbox'][3] + (1 - alpha) * det['bbox'][3]),
            ]
            tr['missed'] = 0; tr['seen'] = tr.get('seen', 0) + 1; tr['last_frame'] = frame_idx
            
            quality = det['score'] * ((tr['bbox'][2]-tr['bbox'][0])*(tr['bbox'][3]-tr['bbox'][1]))
            is_new_best = False
            if quality > tr.get('best_area', 0):
                tr['best_area'] = quality; tr['best_score'] = det['score']
                tr['best_frame'] = bgr.copy(); tr['best_bbox'] = tr['bbox'].copy()
                is_new_best = True
            ...
            if not tr.get('confirmed', False):
                if tr.get('seen', 0) >= confirm_frames and tr.get('best_score', 0) >= 0.35:
                    tr['confirmed'] = True; should_save = True
            elif tr.get('confirmed', False) and is_new_best:
                if det['score'] > (tr.get('saved_score', 0) + 0.05): should_save = True

            if should_save:
                best_bbox = tr.get('best_bbox', tr['bbox'])
                bx1, by1, bx2, by2 = [int(v) for v in best_bbox]
                pad_x = int((bx2-bx1)*ocr_padding_ratio); pad_y = int((by2-by1)*ocr_padding_ratio)
                ...
                if crop.size > 0:
                    ...
                    except: tr['ocr_text'] = "ERR"
                
                # --- DIBUJO PARA LA IMAGEN GUARDADA (Aqui se aplica la etiqueta) ---
                img_to_save = tr['best_frame'].copy()
                # Llamada a la funcion centralizada para quemar etiquetas en la imagen JPG
        ...
        for ti in unmatched_tracks_idx:
            if 0 <= ti < len(tracks): tracks[ti]['missed'] = tracks[ti].get('missed', 0) + 1

        for di in unmatched_dets_idx:
            if di < 0 or di >= len(detections): continue
            det = detections[di]
            tnew = {
                'id': next_track_id, 'bbox': det['bbox'], 'best_frame': bgr.copy(),
                'best_bbox': det['bbox'].copy(), 'best_score': det['score'],
                'best_area': det['score'] * ((det['bbox'][2]-det['bbox'][0])*(det['bbox'][3]-det['bbox'][1])),
                'missed': 0, 'seen': 1 if det['score'] >= THRESHOLD else 0,
                'confirmed': False, 'ocr_text': '', 'last_frame': frame_idx, 'saved_score': 0.0
            }
            tracks.append(tnew); next_track_id += 1
            
            # Confirmación inmediata
            w = tnew['bbox'][2]-tnew['bbox'][0]; h = tnew['bbox'][3]-tnew['bbox'][1]
            if tnew['best_score'] >= 0.80 and (w*h) >= 1200:
                tnew['confirmed'] = True
                bx1, by1, bx2, by2 = tnew['best_bbox']
                crop = tnew['best_frame'][by1:by2, bx1:bx2].copy()
                if crop.size > 0:
                    try:
                        gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)
                ...
                # --- DIBUJO PARA LA IMAGEN GUARDADA (IMMEDIATE) ---
        ...
        for tr in tracks:
            if tr['missed'] > 2: continue
            # Color verde si está confirmado, rojo si no
            color = (0, 255, 0) if tr.get('confirmed', False) else (0, 0, 255)
            # Solo mostrar OCR si está confirmado
            ocr_disp = tr.get('ocr_text', '') if tr.get('confirmed', False) else ""
            
            # Usar la misma función de dibujado para el video
            out_frame = draw_labels(out_frame, tr['bbox'], tr['id'], tr['best_score'], ocr_disp, color)
...
def main():
    ...
    parser.add_argument("--video", required=True, help="Video entrada")
